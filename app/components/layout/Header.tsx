      ...
      <div className="container-custom relative z-10">
        <nav className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2 group">
            <div className={`relative flex items-center justify-center transition-all duration-500 shrink-0 bg-white px-6 py-2 rounded-b-2xl shadow-lg w-40 h-auto`}>
                ...
                {item.submenu ? (
                  <div className="relative">
                    <button
                      ...
                      className={`flex items-center gap-1 px-2 xl:px-4 py-2 text-base font-medium rounded-lg transition-all duration-200 cursor-pointer text-white hover:text-[#d4af37] hover:bg-white/10 ${openDropdown === item.name
                    ...
                    {/* Dropdown Menu */}
                    <div
                      className={`absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-soft-lg border border-secondary-100 overflow-hidden transition-all duration-200 ${openDropdown === item.name
                  ...
                  <Link
                    href={item.href}
                    className="block px-2 xl:px-4 py-2 text-base font-medium rounded-lg transition-all duration-200 text-white hover:text-[#d4af37] hover:bg-white/10"
                  >
                    {item.name}
                  </Link>
                )}
        ...
        {/* Mobile Navigation */}
        <div
          className={`lg:hidden overflow-hidden transition-all duration-300 ${isMobileMenuOpen ? 'max-h-screen mt-4' : 'max-h-0'
            }`}
        >
                ...
                <div key={item.name}>
                  {item.submenu ? (
                    <div>
                      ...
                      <div
                        className={`overflow-hidden transition-all duration-200 ${openDropdown === item.name ? 'max-h-96' : 'max-h-0'
                          }`}
                      >
